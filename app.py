...
with c3:
    st.write("**Isidora Cardenas Ferrada**")
...
def optimizar(f, grad, hess, x0, metodo, max_iter, tol, c1, c2):
        ...
        try:
            ...
            if not np.isfinite(valor) or not np.isfinite(error):
                motivo_detencion = (
                    ...
                    "(infinito o NaN). Esto puede ocurrir cuando la función no está definida "
                    "en alguna zona, el punto inicial es inadecuado o el método avanza hacia "
                    "una región no válida matemáticamente."
                )
                break
            ...
            if error < tol:
                convergio = True
                motivo_detencion = (
                    "El método convergió correctamente, ya que la norma del gradiente "
                    "alcanzó un valor menor que la tolerancia definida."
                )
                break
            ...
            if metodo == "Gradiente":
                ...
                try:
                    ...
                    direccion = -g
                    motivo_detencion = (
                        "La matriz Hessiana no pudo invertirse en alguna iteración. "
                        "Por estabilidad numérica, el método usó dirección de gradiente "
                        "como alternativa."
                    )
            ...
            if np.linalg.norm(x_nuevo - x) < 1e-12 and error >= tol:
                motivo_detencion = (
                    "El proceso se detuvo porque el avance entre iteraciones fue prácticamente "
                    "nulo, pero no se alcanzó la tolerancia solicitada. Esto puede indicar que "
                    "el método quedó estancado o que la función presenta una zona difícil para "
    ...
    if not convergio and motivo_detencion == "":
        motivo_detencion = (
            "El método no alcanzó la tolerancia solicitada dentro del número máximo de "
            "iteraciones definido. Esto no significa necesariamente que la aplicación falló: "
            "puede ocurrir porque la función es compleja, el punto inicial está lejos del "
            "mínimo, el método seleccionado converge lentamente, o porque matemáticamente "
            "la función no posee un mínimo alcanzable bajo las condiciones ingresadas."
        )

    return x, f(x), errores[-1], len(errores), errores, historial, convergio, motivo_detencion
...
def generar_pdf(
    ...
    estado = "Convergió" if convergio else "No convergió dentro de las condiciones definidas"
    ...
    contenido.append(Paragraph("<b>4. Explicación del criterio de parada</b>", estilos["Heading1"]))
    ...
    contenido.append(Paragraph(
        ...
        "del criterio de parada, evitando entregar una solución como óptima cuando matemáticamente "
        "no se ha cumplido la condición de convergencia.",
        estilos["Normal"]
    ))
